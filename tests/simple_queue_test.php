<?php
/**
 * Simple Queue Simulation for Testing
 * Purpose: Test and demonstrate queue management system
 */
    ...
    try {
        switch ($action) {
            case 'create_simulation':
                ...
                if ($queue_result['success']) {
                    echo json_encode([
                        ...
                        'queue_code' => $queue_result['queue_code'] ?? null,
                        'message' => 'Simulation created successfully!'
                    ]);
                } else {
                    echo json_encode([
                        'success' => false,
                        'message' => 'Failed to create queue: ' . $queue_result['message']
                    ]);
                }
                break;
            ...
            case 'simulate_step':
                ...
                // Get current queue entry - look for any status, not just waiting/in_progress
                $stmt = $pdo->prepare("
                ...
                // Handle different simulation scenarios based on current status and step
                switch ($step) {
                    case 'triage':
                        if ($queue_entry['status'] === 'done' && $queue_entry['station_type'] === 'triage') {
                            // Triage already completed, route to consultation
                            ...
                            if ($result) {
                                ...
                                echo json_encode(['success' => false, 'message' => 'Failed to route to consultation']);
                            }
                        } else {
                            // Start and complete triage
                            $queueService->updateQueueStatus($queue_entry_id, 'in_progress', $queue_entry['status'], $employee_id, 'Triage started');
                            $queueService->updateQueueStatus($queue_entry_id, 'done', 'in_progress', $employee_id, 'Vitals: BP 120/80, HR 72');
                            $queueService->routePatientToStation($queue_entry_id, 'consultation', $employee_id, 'Routed to consultation');
                            echo json_encode(['success' => true, 'message' => 'Triage completed, routed to consultation']);
                        }
                        break;
                        
                    case 'consultation':
                        // Get the latest consultation queue entry
                        $stmt = $pdo->prepare("
                            SELECT qe.* FROM queue_entries qe
                            LEFT JOIN stations s ON qe.station_id = s.station_id
                            WHERE qe.appointment_id = ? AND s.station_type = 'consultation'
                            ORDER BY qe.created_at DESC LIMIT 1
                        ");
                        $stmt->execute([$simulation_id]);
                        $consult_entry = $stmt->fetch(PDO::FETCH_ASSOC);
                        
                        if ($consult_entry) {
                            $queueService->updateQueueStatus($consult_entry['queue_entry_id'], 'in_progress', 'waiting', $employee_id, 'Consultation started');
                            $queueService->updateQueueStatus($consult_entry['queue_entry_id'], 'done', 'in_progress', $employee_id, 'Diagnosis: URTI');
                            $queueService->routePatientToStation($consult_entry['queue_entry_id'], 'pharmacy', $employee_id, 'Routed to pharmacy');
                            echo json_encode(['success' => true, 'message' => 'Consultation completed, routed to pharmacy']);
                        } else {
                            echo json_encode(['success' => false, 'message' => 'No consultation queue entry found']);
                        }
                        break;
                        
                    case 'pharmacy':
                        // Get the latest pharmacy queue entry
                        $stmt = $pdo->prepare("
                            SELECT qe.* FROM queue_entries qe
                            LEFT JOIN stations s ON qe.station_id = s.station_id
                            WHERE qe.appointment_id = ? AND s.station_type = 'pharmacy'
                            ORDER BY qe.created_at DESC LIMIT 1
                        ");
                        $stmt->execute([$simulation_id]);
                        $pharmacy_entry = $stmt->fetch(PDO::FETCH_ASSOC);
                        
                        if ($pharmacy_entry) {
                            $queueService->updateQueueStatus($pharmacy_entry['queue_entry_id'], 'in_progress', 'waiting', $employee_id, 'Dispensing started');
                            $queueService->updateQueueStatus($pharmacy_entry['queue_entry_id'], 'done', 'in_progress', $employee_id, 'Medications dispensed');
                            $queueService->routePatientToStation($pharmacy_entry['queue_entry_id'], 'billing', $employee_id, 'Routed to billing');
                            echo json_encode(['success' => true, 'message' => 'Medications dispensed, routed to billing']);
                        } else {
                            echo json_encode(['success' => false, 'message' => 'No pharmacy queue entry found']);
                        }
                        break;
                        
                    case 'billing':
                        // Get the latest billing queue entry
                        $stmt = $pdo->prepare("
                            SELECT qe.* FROM queue_entries qe
                            LEFT JOIN stations s ON qe.station_id = s.station_id
                            WHERE qe.appointment_id = ? AND s.station_type = 'billing'
                            ORDER BY qe.created_at DESC LIMIT 1
                        ");
                        $stmt->execute([$simulation_id]);
                        $billing_entry = $stmt->fetch(PDO::FETCH_ASSOC);
                        
                        if ($billing_entry) {
                            $queueService->updateQueueStatus($billing_entry['queue_entry_id'], 'in_progress', 'waiting', $employee_id, 'Payment processing');
                            $queueService->updateQueueStatus($billing_entry['queue_entry_id'], 'done', 'in_progress', $employee_id, 'Payment: PHP 200');
                            ...
                            echo json_encode(['success' => false, 'message' => 'No billing queue entry found']);
                        }
                        break;
...
<head>
    ...
    <title>Simple Queue Simulation</title>
    <style>
...
<body>
    <div class="container">
        <h1>🔬 Simple Queue Simulation</h1>
        <p>Test the queue management system with a simplified workflow.</p>
    ...
    <script>
        ...
        async function createSimulation() {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    log(`Patient ID: ${result.patient_id}, Queue Code: ${result.queue_code || 'N/A'}`);
            ...
            try {
                ...
                if (result.success) {
                    ...
                    if (currentIndex < steps.length - 1) {
                        ...
                        log('🎉 Simulation completed! Patient visit finished.', 'success');
                    }
                } else {
        ...
        // Initialize
        log('Simple Queue Simulation loaded successfully!', 'success');
    </script>
</body>
</html>