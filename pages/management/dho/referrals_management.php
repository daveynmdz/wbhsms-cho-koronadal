<?php
// referrals_management.php - DHO Side
    ...
    try {
        ...
        if ($dho_district_stmt) {
            ...
            if ($dho_district_result->num_rows === 0) {
                ...
                $error = "Access denied: No facility assignment found for this DHO.";
                $dho_district = null;
            } else {
...
// For DHO users, only show referrals for patients from barangays in their assigned district
if (strtolower($employee_role) === 'dho' && $dho_district !== null) {
...
// Additional security check - if DHO has no district assignment, show no referrals
if (strtolower($employee_role) === 'dho' && $dho_district === null) {
    $referrals = [];
} else {
    try {
...
try {
    // For DHO users, only count referrals for patients from barangays in their assigned district
    if (strtolower($employee_role) === 'dho' && $dho_district !== null) {
        $stmt = $conn->prepare("
            ...
            GROUP BY r.status
        ");
        $stmt->bind_param("i", $dho_district);
    } else {
...
try {
    if (strtolower($employee_role) === 'dho' && $dho_district !== null) {
        // For DHO users, only show barangays in their assigned district
        $stmt = $conn->prepare("SELECT barangay_id, barangay_name FROM barangay WHERE district_id = ? AND status = 'active' ORDER BY barangay_name ASC");
        $stmt->bind_param("i", $dho_district);
    } else {
...
<head>
    ...
    <title>CHO Koronadal — DHO Referrals Management</title>
...
<body>
    ...
    <section class="content-wrapper">
        ...
        <?php if (strtolower($employee_role) === 'dho' && $dho_district !== null): ?>
            <?php
            ...
            $dho_district_name = $district_name_result->fetch_assoc()['district_name'] ?? 'Unknown';
            $district_name_stmt->close();
            ?>
            <div class="alert" style="background-color: #e8f4f8; color: #0c5460; border: 1px solid #b8daff; margin-bottom: 1.5rem;">
                <i class="fas fa-info-circle"></i> 
                <strong>DHO Access:</strong> You are viewing referrals only for patients from barangays in <strong><?php echo htmlspecialchars($dho_district_name); ?></strong> district.
            </div>
        <?php endif; ?>
        ...
        <?php if (!empty($error)): ?>
            ...
            <?php if (strtolower($employee_role) === 'dho' && $dho_district === null): ?>
                <!-- Stop processing if DHO has no district assignment -->
                </section>
                </body>
                </html>
                <?php exit(); ?>
