<?php
// referrals_management.php - BHW Side
...
// Validate session variables to prevent errors
if (empty($_SESSION['employee_id']) || empty($_SESSION['role'])) {
    session_destroy();
    header('Location: ../auth/employee_login.php');
    exit();
}
...
// Get BHW's barangay_id from their facility assignment - only for BHW role
    ...
    try {
        ...
        if ($bhw_barangay_stmt) {
            ...
            if ($bhw_barangay_result->num_rows === 0) {
                ...
                $error = "Access denied: No facility assignment found for this BHW.";
                $bhw_barangay = null;
            } else {
                $row = $bhw_barangay_result->fetch_assoc();
                $bhw_barangay = $row['barangay_id'];
            }
...
// For BHW users, only show referrals for patients from their assigned barangay
if (strtolower($employee_role) === 'bhw' && $bhw_barangay !== null) {
...
// Additional security check - if BHW has no barangay assignment, show no referrals
if (strtolower($employee_role) === 'bhw' && $bhw_barangay === null) {
    $referrals = [];
} else {
    try {
...
try {
    // For BHW users, only count referrals for patients from their assigned barangay
    if (strtolower($employee_role) === 'bhw' && $bhw_barangay !== null) {
        $stmt = $conn->prepare("
            ...
            GROUP BY r.status
        ");
        $stmt->bind_param("i", $bhw_barangay);
    } else {
...
try {
    if (strtolower($employee_role) === 'bhw' && $bhw_barangay !== null) {
        // For BHW users, only show their assigned barangay
        $stmt = $conn->prepare("SELECT barangay_id, barangay_name FROM barangay WHERE barangay_id = ? AND status = 'active'");
        $stmt->bind_param("i", $bhw_barangay);
    } else {
        // For other roles, show all active barangays
...
<body>
    ...
    <section class="content-wrapper">
        ...
        <?php if (strtolower($employee_role) === 'bhw' && $bhw_barangay !== null): ?>
            <?php
            ...
            $bhw_barangay_name = $barangay_name_result->fetch_assoc()['barangay_name'] ?? 'Unknown';
            $barangay_name_stmt->close();
            ?>
            <div class="alert" style="background-color: #e8f4f8; color: #0c5460; border: 1px solid #b8daff; margin-bottom: 1.5rem;">
                <i class="fas fa-info-circle"></i> 
                <strong>BHW Access:</strong> You are viewing referrals only for patients from <strong><?php echo htmlspecialchars($bhw_barangay_name); ?></strong> barangay.
            </div>
        <?php endif; ?>
        ...
        <?php if (!empty($error)): ?>
            ...
            <?php if (strtolower($employee_role) === 'bhw' && $bhw_barangay === null): ?>
                <!-- Stop processing if BHW has no barangay assignment -->
                </section>
                </body>
                </html>
                <?php exit(); ?>
