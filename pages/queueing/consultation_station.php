 ...
 * Purpose: Dedicated consultation station interface for doctors to manage patient consultations and consultation queue
...
// Get consultation station assignment for the current employee
...
$available_consultation_stations = [];

// Get all available consultation stations
$stations_query = "SELECT s.station_id, s.station_name, s.station_type, s.is_active 
                   FROM stations s 
                   WHERE s.station_type = 'consultation' AND s.is_active = 1
                   ORDER BY s.station_name";
$stmt = $pdo->prepare($stations_query);
$stmt->execute();
$available_consultation_stations = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Admin can select any consultation station, others use their assigned station
if (strtolower($employee_role) === 'admin') {
    $can_manage_queue = true;
    
    // If admin specified a station via URL parameter
    if ($selected_station_id) {
        foreach ($available_consultation_stations as $station) {
            if ($station['station_id'] == $selected_station_id) {
                $consultation_station = $station;
                $consultation_station['assignment_id'] = null;
                $consultation_station['employee_id'] = $employee_id;
                break;
            }
        }
    }
    ...
    if (!$consultation_station && !empty($available_consultation_stations)) {
        $consultation_station = $available_consultation_stations[0];
        $consultation_station['assignment_id'] = null;
        $consultation_station['employee_id'] = $employee_id;
    }
} else if ($consultation_station) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax'])) {
    ...
    if (!$can_manage_queue || !$consultation_station) {
        echo json_encode(['success' => false, 'message' => 'Not authorized for consultation operations']);
        exit();
    }
    ...
    try {
        ...
        $station_id = $consultation_station['station_id'];
        
        switch ($action) {
            case 'get_queue_data':
                // Get updated queue data for consultation station
                $consultation_queue = $queueService->getStationQueue($station_id, 'waiting');
                ...
                foreach ($consultation_queue as &$entry) {
                ...
                echo json_encode([
                    'success' => true,
                    'data' => [
                        'waiting_queue' => $consultation_queue,
            ...
            case 'call_next':
                $result = $queueService->callNextPatient('consultation', $station_id, $employee_id);
                if ($result['success']) {
            ...
            case 'skip_patient':
                $reason = $_POST['reason'] ?? 'Patient skipped by consultation staff';
            ...
            case 'force_call':
                $reason = $_POST['reason'] ?? 'Force called by consultation staff';
            ...
            case 'enter_consultation_notes':
                // Redirect to consultation notes page - this will be handled on the frontend
                echo json_encode(['success' => true, 'redirect' => '/pages/clinical-encounter-management/consultation.php?queue_id=' . $queue_entry_id]);
                break;
                
            case 'get_queue_logs':
                $date = $_POST['date'] ?? date('Y-m-d');
                $station_id = $consultation_station['station_id'] ?? null;
...
// Get queue data for consultation station
$consultation_queue = [];
$in_progress_queue = [];
$completed_queue = [];
$skipped_queue = [];
...
<head>
    ...
    <title>Consultation Station - CHO Koronadal</title>
    ...
    <style>
        /* Consultation Station Layout - inheriting dashboard styles */
...
<body>
            ...
            <div class="content-area">
                ...
                <div class="page-header">
                    ...
                    <div class="total-count">
                        <span class="badge bg-info"><?php echo htmlspecialchars($consultation_station['station_name'] ?? 'Consultation Station'); ?></span>
                    ...
                    <div class="section-body">
                        ...
                        <div class="action-section">
                            ...
                            <div class="action-row">
                                ...
                                <?php if (count($available_consultation_stations) > 1): ?>
                                <div class="modern-btn btn-station-switch">
                                    ...
                                    <div class="btn-content">
                                        ...
                                        <select id="stationSelector" onchange="switchStation(this.value)" class="station-select">
                                            ...
                                            <?php foreach ($available_consultation_stations as $station): ?>
                                            <option value="<?php echo $station['station_id']; ?>" 
                                                    <?php echo ($consultation_station && $station['station_id'] == $consultation_station['station_id']) ? 'selected' : ''; ?>>
                ...
                <div class="alert alert-info">
                    ...
                    <span>You are not assigned to a consultation station today. Please contact your supervisor.</span>
                ...
                <div class="parent">
                    ...
                    <div class="div1 card-container">
                        ...
                        <div class="station-info">
                            <div class="info-item">
                                ...
                                <span class="info-value"><?php echo htmlspecialchars($consultation_station['station_name'] ?? 'Consultation Station'); ?></span>
                            </div>
                            <div class="info-item">
                            ...
                            <div class="info-item">
                                ...
                                <span class="info-value">Consultation & Clinical Care</span>
                            </div>
                            <div class="info-item">
                    ...
                    <div class="div4 card-container">
                        ...
                        <div class="actions-grid">
                            <button class="action-btn btn-info" onclick="enterConsultationNotes()" <?php echo !$current_patient ? 'disabled' : ''; ?>>
                                <i class="fas fa-notes-medical"></i> Enter Consultation Notes
                            </button>
                            ...
                            <button class="action-btn btn-success" onclick="rerouteToLab()" <?php echo !$current_patient ? 'disabled' : ''; ?>>
                            ...
                            <button class="action-btn" style="background: linear-gradient(135deg, #fd7e14, #e55a00);" onclick="rerouteToBilling()" <?php echo !$current_patient ? 'disabled' : ''; ?>>
                                <i class="fas fa-receipt"></i> Reroute to Billing
                            </button>
                            <button class="action-btn" style="background: linear-gradient(135deg, #6f42c1, #5a2d91);" onclick="rerouteToDocument()" <?php echo !$current_patient ? 'disabled' : ''; ?>>
                                <i class="fas fa-file-medical"></i> Reroute to Document
                            </button>
                    ...
                    <!-- Div 5: Live Consultation Queue -->
                    <div class="div5 card-container">
                        <div class="section-header">
                            <h4><i class="fas fa-users"></i> Consultation Queue (<?php echo count($consultation_queue); ?>)</h4>
                        </div>
                        <div class="section-body" style="padding: 15px 20px;">
                        <?php if (!empty($consultation_queue)): ?>
                        <table class="queue-table">
                            ...
                            <tbody>
                                <?php foreach ($consultation_queue as $patient): ?>
                                <tr>
    ...
    <script>
        ...
        // Function to open public consultation display in popup window
        function openPublicDisplay() {
            ...
            if (publicDisplayWindow && !publicDisplayWindow.closed) {
                ...
                showAlert('Public consultation display is already open', 'info');
                return;
            }
            ...
            try {
                // Open the public consultation display window
                publicDisplayWindow = window.open(
                    'public_display_consultation.php',
                    'ConsultationPublicDisplay',
                    windowFeatures
                );
                ...
                if (publicDisplayWindow) {
                    ...
                    publicDisplayWindow.addEventListener('load', function() {
                        try {
                            publicDisplayWindow.document.title = 'Consultation Queue Display - CHO Koronadal';
                        } catch (e) {
                    ...
                    publicDisplayWindow.addEventListener('beforeunload', function() {
                        console.log('Public consultation display window closing');
                        publicDisplayWindow = null;
                        updateDisplayButtonState(false);
                    });
                    
                    // Show success message
                    showAlert('Public consultation display opened successfully', 'success');
                ...
                console.error('Error opening public consultation display:', error);
        ...
        function enterConsultationNotes() {
            if (!currentPatient) {
                showAlert('No patient currently in consultation', 'error');
                return;
            }
            
            // Redirect to consultation notes page
            window.location.href = '/pages/clinical-encounter-management/consultation.php?queue_id=' + currentPatient.queue_entry_id;
        }
        ...
        function rerouteToLab() {
            ...
            const remarks = prompt('Remarks for lab referral:') || 'Completed consultation, forwarded to laboratory';
        ...
        function rerouteToPharmacy() {
            ...
            const remarks = prompt('Remarks for pharmacy referral:') || 'Completed consultation, forwarded to pharmacy';
            performQueueAction('reroute_to_pharmacy', currentPatient.queue_entry_id, 'Forwarding to pharmacy...', { remarks: remarks });
        }
        
        function rerouteToBilling() {
            if (!currentPatient) {
                showAlert('No patient currently in consultation', 'error');
                return;
            }
            
            const remarks = prompt('Remarks for billing referral:') || 'Completed consultation, forwarded to billing';
            performQueueAction('reroute_to_billing', currentPatient.queue_entry_id, 'Forwarding to billing...', { remarks: remarks });
        }
        ...
        // Consultation-specific helper functions
        ...
        function viewReferral() {
            ...
            // Open referral details if available
            if (currentPatient.referral_id) {
                window.open('/pages/management/admin/referrals/referral_details.php?referral_id=' + currentPatient.referral_id, '_blank');
            } else {
                showAlert('No referral information available for this patient', 'info');
            }
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            // Station configuration for consultation station
            const stationConfig = {
                stationType: 'consultation',
                stationId: <?php echo json_encode($consultation_station['station_id'] ?? null); ?>,
                stationName: <?php echo json_encode($consultation_station['station_name'] ?? 'Consultation Station'); ?>,
                ...
                ajaxUrl: 'consultation_station.php',
                refreshInterval: 5000,
                nextStations: ['billing', 'lab', 'pharmacy', 'document'],
            ...
            if (typeof StationManager !== 'undefined') {
                ...
                console.log('✅ Consultation Station Manager initialized successfully');
            } else {
